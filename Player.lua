...
function Player:checkRightCollision()
    if self.dx > 0 then
        ...
        if self.map:collides(self.map:tileAt(self.x + self.width, self.y)) or
            ...
            -- Check if Play Level is completed
            if checkLevelCompleted(self.map:tileAt(self.x + self.width, self.y)) then
...
function checkLevelCompleted(tile)

    -- Flags are colliables that trigger
    -- Level changes
    local levelCollidables = {
        FLAG_POST_TOP, FLAG_POST_MIDDLE, FLAG_POST_BOTTOM
    }

    -- iterate and return true if our tile type matches
    for _, v in ipairs(levelCollidables) do
        if tile.id == v then
            return true
        end
    end
